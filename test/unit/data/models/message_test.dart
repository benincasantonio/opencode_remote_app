...
void main() {
  group('Message', () {
    test('parses the messages fixture into typed messages', () {
      final messages = loadFixtureList('messages_full')
          .map((json) => MessageWithParts.fromJson(json as Map<String, dynamic>))
          .toList();
    ...
    test('parses user message fields', () {
      final user = (loadFixtureList('messages_full')
              .map((json) => MessageWithParts.fromJson(json as Map<String, dynamic>))
              .toList()
              .first
              .info as UserMessage);
    ...
    test('parses assistant message fields', () {
      final assistant = (loadFixtureList('messages_full')
              .map((json) => MessageWithParts.fromJson(json as Map<String, dynamic>))
              .toList()[1]
              .info as AssistantMessage);
    ...
    test('parses assistant message parts', () {
      ...
      expect(
        types,
        containsAll([StepStartPart, ReasoningPart, TextPart, ToolPart, StepFinishPart]),
      );
      ...
      expect((completed.state as ToolStateCompleted).title, 'Users/antoniobenincasa');
    ...
    test('throws on unknown role', () {
      expect(
        () => Message.fromJson({'role': 'system', 'id': 'msg_1', 'sessionID': 'ses_1'}),
  ...
  group('OutputFormat', () {
    test('parses text', () {
      expect(OutputFormat.fromJson({'type': 'text'}), const OutputFormat.text());
    });
    ...
    test('round-trips json_schema', () {
      const format = OutputFormat.jsonSchema(schema: {'type': 'object'}, retryCount: 2);
    ...
    test('throws on unknown type', () {
      expect(() => OutputFormat.fromJson({'type': 'mystery'}), throwsA(isA<CheckedFromJsonException>()));
    });
  });
